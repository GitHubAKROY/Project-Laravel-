...
class ReportController extends Controller {
    ...
    public function __construct() {
        ...
        //     $accountingReports = ['reports.generalLedger', 'reports.trialBalance', 'reports.profitAndLoss', 'reports.balanceSheet'];
        //     $route_name = request()->route()->getName();

        //     if ($request->tenant->package->accounting != 1 && in_array($route_name, $accountingReports)) {
        //         if ($request->ajax()) {
    ...
    public function loan_due_report(Request $request) {
        ...
        $data['report_data'] = LoanRepayment::selectRaw('loan_id, SUM(amount_to_pay) as total_due')
            ->with('loan')
    ...
    public function expense_report(Request $request) {
        if ($request->isMethod('get')) {
            return view('backend.admin.reports.expense_report');
        } else if ($request->isMethod('post')) {
            @ini_set('max_execution_time', 0);
            @set_time_limit(0);
