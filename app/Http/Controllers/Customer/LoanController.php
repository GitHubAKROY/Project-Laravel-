...
class LoanController extends Controller
{
    ...
    public function index()
    {
        $loans = Loan::where('borrower_id', auth()->user()->member->id)
            ...
            ->get();
        return view('backend.customer.loan.my_loans', compact('loans'));
    }
        ...
        if (request()->isMethod('get')) {
            ...
            if ($loan->total_paid >= $loan->applied_amount) {
                ...
                if ($repayment->principal_amount != $existing_amount) {
                    ...
                    $calculator    = new Calculator(
                        in_array($interest_type, ['flat_rate', 'fixed_rate']) ? $loan->applied_amount : $loan->applied_amount - $loan->total_paid,
